package com.azxc.unified.common.file;
...
public class FileUpload {
  ...
  public static Upload getFile(MultipartFile multipartFile, String modulePath) {
    ...
    upload.setMine(multipartFile.getContentType());
    upload.setSize(multipartFile.getSize());
    upload.setName(FileUpload.genFileName(multipartFile.getOriginalFilename()));
    upload.setPath(
        getPathPattern() + modulePath + FileUpload.genDateMkdir("yyyyMMdd") + upload.getName());
    return upload;
  }
   ...
   * @param upload        Upload
   */
  public static void transferTo(MultipartFile multipartFile, Upload upload)
      throws IOException, NoSuchAlgorithmException {
    byte[] buffer = new byte[4096];
    MessageDigest md5 = MessageDigest.getInstance("MD5");
    MessageDigest sha1 = MessageDigest.getInstance("SHA1");
    try (OutputStream fos = Files
        .newOutputStream(getDestFile(upload).toPath()); InputStream fis = multipartFile
        .getInputStream()) {
      int len = 0;
